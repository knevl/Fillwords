...
uses
  ...
  Buttons, Menus, MaskEdit, PairSplitter, ComCtrls, Interfaces, uplaysound,
  mmsystem;
...
type
  ...
  TApp_Fillword = class(TForm)
    ...
    procedure text_edit_rightClick(Sender: TObject);
...
procedure TApp_Fillword.text_edit_rightClick(Sender: TObject);
begin
...
begin
     ...
     group_chooselvl.Visible:=false;
     group_go.Visible:=false;
     if group_settings.Visible then
        begin
        hello_words.Visible:=true;
        group_settings.Visible:=false;
        end
